"""Experiment 46: rolling EH mean gate for gateway ef-gap on DEEP-96 cluster drift."""
...
def run_gateway_rolling_eh(dataset, index_path, cfg):
    k = cfg["k"]
    ef_values = cfg["ef_values"]
    ef_low = cfg["ef_low"]
    ef_high = cfg["ef_high"]
    ...
    eh_gate_k = cfg["eh_gate_k"]
    min_window_fill = cfg["min_window_fill"]
    base = dataset["base"]
    ...
    print(f"  Calibrating rolling EH gate on first {n_calib} epochs (ef={ef_low})...")
    calib_eh = []
    index.set_ef(ef_low)
    for i in range(n_calib):
        queries = dataset["epochs"][i]
    ...
    calib_mean = float(np.mean(calib_eh))
    calib_std = float(np.std(calib_eh))
    gate_threshold = calib_mean + eh_gate_k * calib_std
    print(f"  calib_mean={calib_mean:.4f}  calib_std={calib_std:.4f}  "
          f"gate_threshold={gate_threshold:.4f} (k={eh_gate_k})")

    # --- Main loop ---
    eh_window = collections.deque(maxlen=rolling_window_size)
    total_edges = 0
    rows = []
    ...
    for epoch, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        window_mean = float(np.mean(eh_window)) if len(eh_window) > 0 else 0.0
        gate_open = (len(eh_window) >= min_window_fill) and (window_mean > gate_threshold)
        ...
        for i, q in enumerate(queries):
            ...
            # Gate is open: run ef_high, compute gap, insert edges.
        ...
        rows.append({"condition": "gateway_rolling_eh", "epoch": epoch, "ef": ef_low,
        ...
        for ef in ef_values:
            ...
            index.set_ef(ef)
            ids, _ = index.knn_query(queries, k=k, num_threads=1)
            rows.append({"condition": "gateway_rolling_eh", "epoch": epoch, "ef": ef,
                         "recall": _compute_recall(ids, gt, k), "edge_count": total_edges,
                         "gate_open": gate_open})

        print(f"  epoch {epoch:2d}  recall@{ef_low}={recall_low:.4f}  "
              f"gate_open={gate_open}  window_fill={len(eh_window)}  "
              f"window_mean={window_mean:.4f}  edges={total_edges}")
...
def main():
    ...
    for schedule, ds_key, results_key in [
        ...
        print(f"\n=== {schedule} — Gateway Rolling EH ===")
        rows.extend(run_gateway_rolling_eh(dataset, index_path, cfg))
