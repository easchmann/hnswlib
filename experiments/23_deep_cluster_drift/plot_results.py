"""Experiment 23: plot recall and edge-accumulation results."""
...
RESULTS_DIR = ROOT / "experiments" / "23_deep_cluster_drift"
PLOTS_DIR = Path(__file__).parent / "plots"
DATASET_GRADUAL = ROOT / "experiments" / "23_deep_cluster_drift" / "dataset_gradual"
...
def _get_schedule_t(cfg, schedule_tag):
    return cfg["drift"][f"schedule_{schedule_tag}"]
...
def figure1_gradual(cfg):
    ...
    ax.set_title("DEEP-1B cluster drift (gradual) — recall@10 vs epoch")
    ax.legend()

    if cluster_diag is not None:
        cd = cluster_diag.get("cluster_diagnostics", {})
        sd = cluster_diag.get("selection_diagnostics", {})
        chosen_idx = sd.get("chosen_cluster_idx")
        sizes = cd.get("cluster_sizes", [])
        far_size = sizes[chosen_idx] if chosen_idx is not None and chosen_idx < len(sizes) else "?"
        far_dist = sd.get("chosen_distance_to_reference", float("nan"))
        ann = f"far-cluster size={far_size}, dist to entry point={far_dist:.2f}"
        ax.text(
            0.99, 0.02, ann,
...
def figure2_sudden(cfg):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Recall@10")
    ax.set_title("DEEP-1B cluster drift (sudden) — recall@10 vs epoch")
    ax.legend()
...
def figure4_edge_accumulation(cfg):
    ...
    fig.suptitle("Edge accumulation and EH under DEEP-1B cluster drift (gradual)")
    fig.tight_layout()
