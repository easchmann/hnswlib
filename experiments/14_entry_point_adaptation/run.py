"""Experiment 14: entry point adaptation vs conjugate-only baseline."""
...
def run_adaptive(dataset, index_path, base, config, condition):
    """Conjugate graph adaptation with optional entry point adaptation."""
    ...
    mgr_config = {
        ...
        "entry_point_adaptation": condition.get("entry_point_adaptation", False),
    }
    ...
    for epoch_idx, (queries, gt) in enumerate(
        ...
        print(
            ...
            + (f"  ep_node={ep['entry_point_node']}" if ep.get("entry_point_node") is not None else "")
        )
        ...
        for ef in ef_values:
            ...
            results.append({
                ...
                "entry_point_node": ep.get("entry_point_node"),
            })
