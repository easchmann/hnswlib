"""Experiment 27: layer-0 eviction + EH repair vs conjugate EH vs static."""
...
def run_hybrid_eh(dataset, index_path, cfg):
    """Conjugate EH + simultaneous layer-0 eviction injection from the same repair signal."""
    k = cfg["k"]
    ef_values = cfg["ef_values"]
    primary_ef = cfg["primary_ef"]
    n_calib = cfg["eh_n_calibration_epochs"]
    ...
    for epoch, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        # Inject layer-0 edges for the same repair nodes that just received conjugate edges.
        # ep["repair_stats"] is not None only when drift was detected, cooldown was ok,
        # and repair_nodes was non-empty — the exact condition under which we want to inject.
        if ep["repair_stats"] is not None:
            rn = find_repair_candidates(
                ep["hot_cells"], cell_labels, mgr.node_eh_accumulator,
                eh_threshold_percentile=cfg["eh_threshold_percentile"],
                max_nodes=cfg["eh_max_repair_nodes"],
                last_repaired=None, use_diversity=False)
            if len(rn):
                cands = compute_candidate_edges(
                    rn, index, base,
                    M_candidates=cfg["eh_M_candidates"],
                    ef_search=cfg["eh_ef_repair"],
                    queries=queries, result_ids=primary_ids)
                for src, cand_list in cands.items():
                    for dst, _ in cand_list:
                        l0_count += index.add_layer0_edge_evict(int(src), int(dst))

        print(f"  epoch {epoch:2d}  recall@{primary_ef}="
              f"{_compute_recall(primary_ids, gt, k):.4f}  "
              f"drift={ep['drift_detected']}  conj={cg._total_edges}  l0={l0_count}")

        for ef in ef_values:
            ids, _ = ef_results[ef]
            rows.append({"condition": "hybrid_eh", "epoch": epoch, "ef": ef,
                         "recall": _compute_recall(ids, gt, k),
                         "edge_count": cg._total_edges,
                         "l0_edge_count": l0_count})
    return rows
...
def main():
    ...
    for schedule, ds_key, results_key in [
        ...
        print(f"\n=== {schedule} — Hybrid EH (conjugate + layer-0) ===")
        rows.extend(run_hybrid_eh(dataset, index_path, cfg))
