"""Experiment 03: ablation study for adaptive method improvements."""
...
def run_condition(dataset, index_path, base, config, condition):
    """Run one ablation condition; returns list of result rows."""
    ...
    mgr_config = {
        ...
        "max_repair_nodes": condition["max_repair_nodes"],
        "rng_relaxation": adapt_cfg["rng_relaxation"],
        "n_epochs": len(dataset["epochs"]),
        "use_diversity": condition["diversity"],
        "two_hop": condition["two_hop"],
    }
    ...
    for epoch_idx, (queries, gt) in enumerate(
        ...
        # Evaluate at each ef; all share the same conjugate graph snapshot
        ef_results = {}
        for ef in ef_values:
        ...
        # Adaptation driven by primary_ef results
...
def run_all(config_path):
    ...
    for condition in cfg["ablation_conditions"]:
        name = condition["name"]
        print(f"\n{'='*60}")
        print(
            f"CONDITION: {name}  "
            f"(max_repair_nodes={condition['max_repair_nodes']}, "
            f"diversity={condition['diversity']}, two_hop={condition['two_hop']})"
        )
        print("=" * 60)
