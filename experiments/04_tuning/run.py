"""Experiment 04: targeted follow-up tuning of adaptive method parameters."""
...
def run_condition(dataset, index_path, base, config, condition):
    """Run one tuning condition; returns list of result rows."""
    ...
    mgr_config = {
        ...
        "eh_threshold_percentile": condition["eh_threshold_percentile"],
...
def run_all(config_path):
    ...
    for condition in cfg["tuning_conditions"]:
        name = condition["name"]
        print(f"\n{'='*60}")
        print(
            f"CONDITION: {name}  "
            f"(max_repair_nodes={condition['max_repair_nodes']}, "
            f"eh_threshold_percentile={condition['eh_threshold_percentile']}, "
