"""Exp 35 plots: recall and edge-count curves for static vs current vs improved adapter."""
...
def plot_schedule(df, schedule, cfg, plots_dir):
    ...
    fig.suptitle(f"SIFT rotation drift — improved adapter ({schedule})", fontsize=11)
    fig.tight_layout()
    ...
    ax.set_title(f"Edge count — {schedule} drift (SIFT rotation drift — improved adapter)")
