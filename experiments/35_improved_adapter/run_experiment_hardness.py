"""Experiment 35 on hardness drift: static vs adaptive_current vs adaptive_improved.

Reuses the exp33 dataset (saved with save_cluster_drift_dataset) and pre-built hardness index. Adds adaptive_improved as a third condition alongside exp33's two.
"""
...
def main():
    config_path = sys.argv[1] if len(sys.argv) > 1 else str(EXP_DIR / "config_hardness.yaml")
    cfg = _load_config(config_path)

    # The real index vectors; mmap so RSS stays low until accessed
    ...
    for schedule_name, dataset_key, out_path in schedules:
        ...
        dataset["base"] = base  # override placeholder with real index vectors

        index_path = cfg["index_path"]  # absolute path; do not prepend ROOT
