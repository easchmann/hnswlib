"""Exp 35 hardness mmd2gate: static vs adaptive_current vs adaptive_improved_mmd2gate."""
...
def run_adaptive_current(dataset, index_path, cfg):
    ...
    for epoch_idx, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        for ef in ef_values:
            ...
            ef_results[ef] = {"ids": ids, "dists": dists,
                              "recall": _compute_recall(ids, gt, k)}
...
def run_adaptive_improved_mmd2gate(dataset, index_path, cfg):
    """ImprovedAdaptationManager with mmd2_gate=True."""
    k = cfg["k"]
    ef_values = cfg["ef_values"]
    primary_ef = cfg["primary_ef"]
    n_calib = cfg["n_calibration_epochs"]
    ...
    print("  [adaptive_improved_mmd2gate] Building spatial index...")
    centroids, cell_labels = build_spatial_index(base, n_cells=cfg["n_cells"], seed=42)
    ...
    mgr_cfg = {
        ...
        "mmd2_gate":                          cfg.get("mmd2_gate", False),
    }
    ...
    print(f"  [adaptive_improved_mmd2gate] Calibrating on first {n_calib} epochs...")
    ...
    for epoch_idx, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        for ef in ef_values:
            ...
            rows.append({
                "condition":      "adaptive_improved_mmd2gate",
                "epoch":          epoch_idx,
                "ef":             ef,
                "recall":         recall,
                "edge_count":     cg._total_edges,
        ...
        print(
            f"  [adaptive_improved_mmd2gate] epoch {epoch_idx:2d}"
            f"  recall@{primary_ef}={primary_recall:.4f}"
            f"  edges_added={stats['total_edges_added']}"
            f"  repairs={stats['repair_count']}"
            f"  edges_total={cg._total_edges}"
...
def main():
    config_path = (
        ...
        else str(EXP_DIR / "config_hardness_mmd2gate.yaml")
    )
    ...
    for schedule_name, dataset_key, out_path in schedules:
        ...
        all_rows.extend(run_adaptive_improved_mmd2gate(dataset, index_path, cfg))
