"""Experiment 47: rolling ef-gap mean gate for gateway ef-gap on DEEP-96 cluster drift."""
    ...
    for epoch, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        for ef in ef_values:
            ...
            rows.append({"condition": "static", "epoch": epoch, "ef": ef,
                         ...
                         "gate_open": False, "window_mean_gap": 0.0, "n_efhigh_queries": 0})
    ...
    for epoch, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        for ef in ef_values:
            ...
            rows.append({"condition": "adaptive_eh", "epoch": epoch, "ef": ef,
                         ...
                         "window_mean_gap": 0.0, "n_efhigh_queries": 0})
    return rows


def run_gateway_gap_gate(dataset, index_path, cfg):
    ...
    for epoch, (queries, gt) in enumerate(zip(dataset["epochs"], dataset["groundtruth"])):
        ...
        if gap_gate_threshold is None:
            ...
            gate_open = (len(gap_window) >= min_window_fill) and (window_mean > gap_gate_threshold)
        ...
        for i, q in enumerate(queries):
            ...
            if epoch < n_calib:
                ...
                # Post-calibration: update rolling window and insert edges.
        ...
        # After last calibration epoch: compute threshold from accumulated gaps.
        if epoch == n_calib - 1 and calib_gap_vals:
            ...
            gap_gate_threshold = calib_mean_gap + gap_gate_k * calib_std_gap
            print(f"  calib_mean_gap={calib_mean_gap:.2f}  calib_std_gap={calib_std_gap:.2f}  "
                  f"gap_gate_threshold={gap_gate_threshold:.2f}  "
        ...
        rows.append({"condition": "gateway_gap_gate", "epoch": epoch, "ef": ef_low,
        ...
        for ef in ef_values:
            ...
            index.set_ef(ef)
            ids, _ = index.knn_query(queries, k=k, num_threads=1)
            rows.append({"condition": "gateway_gap_gate", "epoch": epoch, "ef": ef,
        ...
        print(f"  epoch {epoch:2d}  recall@{ef_low}={recall_low:.4f}  gate_open={gate_open}  "
              f"window_mean_gap={window_mean_now:.2f}  n_efhigh={n_efhigh_epoch}  "
              f"edges={total_edges}")
...
def main():
    ...
    for schedule, ds_key, results_key in [
        ...
        print(f"\n=== {schedule} — Gateway Gap Gate ===")
        rows.extend(run_gateway_gap_gate(dataset, index_path, cfg))
