...
#
# Usage:
#   python plot_continuous_drift.py --results_dir results_continuous_drift_large
#   python plot_continuous_drift.py --results_dir results_continuous_drift_large --plot_ef 200
...
def plot_recall_surface():
    ...
    fig.suptitle(
        ...
        f"index on blob (σ={cluster_std}), queries shifted along fixed direction",
        y=1.01
    )
    fig.tight_layout()
...
def plot_lb_traces(ef, max_panels=6):
    df = traces[traces["ef_search"] == ef]
    if df.empty:
        return

    # pick a spread of shift levels to keep the figure readable
