# Generate figures from the CSVs produced by experiment_continuous_drift.py.
#
# Usage:
#   python plot_continuous_drift.py --results_dir DIR --plot_ef 200
...
shifts = sorted(summary["shift"].unique())
...
def get(shift, ef, col):
    row = summary[(summary["shift"] == shift) & (summary["ef_search"] == ef)]
    if row.empty:
        return float("nan")
...
def plot_recall_surface():
    ...
    for ef, color in zip(efs, ef_cmap(len(efs))):
        recalls = [get(s, ef, "mean_recall") for s in shifts]
        ax1.plot(shifts, recalls, color=color, marker="o", markersize=5,
    ...
    for shift, color in zip(shifts, shift_cmap(len(shifts))):
        recalls = [get(shift, ef, "mean_recall") for ef in efs]
    ...
    ax2.legend(fontsize=8, title="shift", loc="upper left")
    ax2.grid(alpha=0.3)

    fig.suptitle(
        f"continuous distribution shift — Recall@{k}  [{mode} mode]\n"
        f"index on blob (shift={cluster_std}), queries shifted along fixed direction",
        y=1.01
    )
    fig.tight_layout()
...
def plot_metrics(ef):
    ...
    for ax, (col, title, ylabel) in zip(axes, metrics):
        if col not in summary.columns:
            ...
            continue
        vals = [get(s, ef, col) for s in shifts]
        ax.plot(shifts, vals, color="#4C72B0", marker="o", lw=2)
        # recolour each point by shift level so the gradient is visible
        for s, v, c in zip(shifts, vals, colors):
    ...
    fig.suptitle(f"metrics vs shift  [ef={ef}, {mode}]", y=1.01)
    fig.tight_layout()
...
def plot_violin(ef):
    ...
    colors = shift_cmap(len(shifts))
    fig, ax = plt.subplots(figsize=(max(8, len(shifts) * 0.9), 5))

    data   = [df[df["shift"] == s]["recall"].tolist() for s in shifts]
    labels = [f"shift={s}" for s in shifts]

    parts = ax.violinplot(data, positions=range(len(shifts)),
...
def plot_lb_traces(ef):
    ...
    colors = shift_cmap(len(all_shifts))

    fig, axes = plt.subplots(1, len(all_shifts),
                             figsize=(3.5 * len(all_shifts), 4))
    if len(all_shifts) == 1:
        axes = [axes]

    # pre-compute all subsets to derive shared axis limits first
    all_subs = [df[df["shift"] == s].sort_values("iteration") for s in all_shifts]
    ...
    for ax, shift, color, sub in zip(axes, all_shifts, colors, all_subs):
        ax.plot(sub["iteration"], sub["mean_lowerbound"], color=color, lw=2)
        ax.set_title(f"shift={shift}", color=color)
