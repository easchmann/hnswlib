...
# --- plot 2: recall delta (poolAndRewire - no_adaptation) vs bin -------------

def plot_recall_delta():
    if "no_adaptation" not in strategies or "poolAndRewire" not in strategies:
    ...
    for ax, ef in zip(axes, efs):
        deltas = [
            get("poolAndRewire", ef, b, "mean_recall") -
    ...
    axes[0].set_ylabel(f"ΔRecall@{k}  (PoolAndRewire − baseline)")
    fig.suptitle(
