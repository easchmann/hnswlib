"""Exp 50 plots: served recall, standard recall, edges, ef-scaling, and ratio signal."""
...
COLORS = {
    ...
    "ef_escalate": "darkorange",
    "ef_escalate_gateway": "firebrick",
}
LABELS = {
    "static": "Static",
    "adaptive_eh": "Adaptive EH (conjugate)",
    "ef_escalate": "Ef escalation (ratio gate)",
    "ef_escalate_gateway": "Ef escalation + gateway edges",
}
COND_ORDER = ["static", "adaptive_eh", "ef_escalate", "ef_escalate_gateway"]
...
def _gate_open_epochs(df):
    """Return set of epochs where gate_open=True, using ef_escalate (or ef_escalate_gateway)."""
    for cond in ("ef_escalate", "ef_escalate_gateway"):
        sub = df[(df["condition"] == cond) & df["gate_open"]]
        if len(sub) > 0:
...
def _served_recall_series(df, cond):
    ...
    # ef_escalate / ef_escalate_gateway: find row where ef == served_ef per epoch
    result = {}
...
def plot_schedule(df, schedule, cfg, plots_dir):
    ...
    fig.suptitle(f"Recall@10 — {schedule} drift (DEEP-96 cluster, exp50)", fontsize=11)
    fig.tight_layout()
    ...
    for band_start, band_end in bands:
        ax.axvspan(band_start - 0.5, band_end + 0.5,
                   color="darkorange", alpha=0.10, zorder=0)
    for cond in ("adaptive_eh", "ef_escalate_gateway"):
        if cond not in present:
            continue
    ...
    # Figure 5: gap ratio signal and ef escalation trigger
    ...
    ratio_sub = df[(df["condition"] == "ef_escalate") &
    ...
    ax1.set_title(f"Gap ratio signal and ef escalation trigger — {schedule} drift")
    fig.tight_layout()
    out = plots_dir / f"{schedule}_signal.pdf"
    fig.savefig(out)
    plt.close(fig)
    print(f"  saved {out}")
