"""Exp 52 plots: recall and edge-count curves for static vs current vs improved adapter
on the clustered structural hardness drift dataset."""
...
def _plateau_summary(df, cfg, schedule):
    """Print plateau statistics for epochs 20-24 at primary ef."""
...
def plot_schedule(df, schedule, cfg, plots_dir):
    ...
    fig.suptitle(f"Clustered structural hardness drift — improved adapter ({schedule})", fontsize=11)
    fig.tight_layout()
    ...
    ax.set_title(
        ...
        f"(clustered structural hardness drift — improved adapter)"
    )
