"""Build 50-epoch clustered structural hardness drift dataset for exp52/50ep.

Same hard-pool selection as the parent exp52 (three-way filter + best-cluster selection),
but with a 50-epoch schedule: the gradual plateau is extended from ep20–24 to ep20–49
and the sudden plateau from ep13–24 to ep13–49.  The parent datasets are not touched.
"""
...
from src.drift.hardness_drift import build_hardness_drift_dataset


def _build_50ep_schedule(schedule_name):
    """50-epoch hardness drift schedules.

    Gradual: 5 pre-drift + ramp 0→0.5 (5) + plateau 0.5 (5) + ramp 0.5→1.0 (5) + plateau 1.0 (30) = 50
    Sudden:  13 pre-drift (t=0) + 37 full-drift (t=1.0) = 50
    """
    ...
    if schedule_name == "sudden":
        return [0.0] * 13 + [1.0] * 37
...
def main():
    ...
    if (os.path.exists(os.path.join(out_gradual, "config.json"))
        ...
        print("50ep datasets already exist, skipping")
        return
    ...
    assert n_epochs == 50, f"This script is for n_epochs=50; got {n_epochs}"
    ...
    for schedule_name in ["gradual", "sudden"]:
        schedule_t = _build_50ep_schedule(schedule_name)
        assert len(schedule_t) == 50, f"Expected 50 t values, got {len(schedule_t)}"

        print(f"\nStep D — building {schedule_name} dataset (50 epochs)...")
