"""Exp 38 plots: recall and edge-count curves for static vs current vs improved adapter
on the structural hardness drift dataset."""
...
def plot_schedule(df, schedule, cfg, plots_dir):
    ...
    fig.suptitle(f"Structural hardness drift — improved adapter ({schedule})", fontsize=11)
    fig.tight_layout()
    ...
    # Figure 2: cumulative edge count vs epoch for the two adaptive conditions
    ...
    ax.set_title(f"Edge count — {schedule} drift (structural hardness drift — improved adapter)")
