...
def validate_monotone(diagnostics, label):
    # warn if any metric decreases as t increases — that would be suspicious
...
def build(config_path, schedule_key):
    ...
    assert len(schedule) == drift["n_epochs"], \
        f"Schedule length {len(schedule)} != n_epochs {drift['n_epochs']}"

    print(f"Building drift sequence ({schedule_key}, {drift['n_epochs']} epochs)...")
    ...
    save_drift_dataset(str(out_path), base, epochs, groundtruth_per_epoch,
                       {**cfg, "schedule_used": schedule_key}, diagnostics)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default=str(Path(__file__).parent / "config.yaml"))
    parser.add_argument("--schedule", choices=["schedule_gradual", "schedule_sudden"],
