"""Experiment 16 figures: DEEP-96 cluster-reweighted drift."""
...
CONDITION_COLORS = {
    ...
    "adaptive_naf": "forestgreen",
    "adaptive_naf_exact": "mediumseagreen",
}
CONDITION_LABELS = {
    "static": "Static HNSW",
    "periodic_rebuild": "Periodic rebuild (every 5 ep.)",
    "adaptive_mconj48": "Adaptive EH ($M_{\\mathrm{conj}}=48$)",
    "adaptive_naf": "Adaptive NAF, HNSW pool ($M_{\\mathrm{conj}}=48$)",
    "adaptive_naf_exact": "Adaptive NAF, exact pool ($M_{\\mathrm{conj}}=48$)",
}
...
def plot_figure1(data, cfg, plots_dir):
    ...
    ax.set_title("DEEP-96 cluster-reweighted drift (gradual) — recall@10 vs epoch",
...
def plot_figure2(data, cfg, plots_dir):
    ...
    ax.set_xlabel("Epoch", fontsize=11)
    ax.set_ylabel("Recall@10", fontsize=11)
    ax.set_title("DEEP-96 cluster-reweighted drift (sudden) — recall@10 vs epoch",
                 fontsize=12, fontweight="bold")
    ax.set_xlim(-0.5, 24.5)
    ax.set_xticks(range(0, 25, 2))
    ax.set_ylim(0, 1.05)
...
def plot_figure3(cfg, plots_dir):
    ...
    fig.suptitle("DEEP-96 cluster drift diagnostics (gradual)", fontsize=12, fontweight="bold")
    fig.tight_layout()
