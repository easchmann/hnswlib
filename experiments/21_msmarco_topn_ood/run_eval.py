"""Experiment 21: MS MARCO top-N OOD — static vs adaptive across pool sizes."""
...
def run_static(dataset, index_path, config, top_n, schedule_name):
    ...
    for epoch_idx, (queries, gt) in enumerate(
        ...
        print(
            f"  [top_n={top_n}] epoch {epoch_idx:2d}  "
            f"recall@{primary_ef}={results[-len(ef_values)]['recall_at_k']:.4f}"
        )

    return results


def run_adaptive(dataset, index_path, config, condition, top_n, schedule_name):
...
def run_all(config_path):
    ...
    with open(str(results_dir / "_config.json"), "w") as f:
        json.dump(cfg, f, indent=2)

    top_n_values = cfg["focused_ood"]["top_n_values"]
    schedule_tags = ["gradual", "sudden"]

    for top_n in top_n_values:
        for schedule_tag in schedule_tags:
            dataset_path = str(results_dir / f"dataset_top{top_n}_{schedule_tag}")
            print(f"\n{'#'*70}")
            print(f"TOP_N={top_n}  SCHEDULE={schedule_tag.upper()}")
            print("#" * 70)
            ...
            for condition in cfg["conditions"]:
                ...
                print(f"\n{'='*60}")
                print(f"CONDITION: {name}  (top_n={top_n}, {schedule_tag})")
                print("=" * 60)

                if method == "static":
                    rows = run_static(dataset, index_path, cfg, top_n, schedule_tag)
                elif method == "adaptive":
                    rows = run_adaptive(dataset, index_path, cfg, condition, top_n, schedule_tag)
                else:
                    raise ValueError(f"Unknown method: {method}")

                out_csv = str(results_dir / f"top{top_n}_{schedule_tag}_{name}.csv")
