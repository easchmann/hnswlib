"""Experiment 17: main evaluation — static vs periodic rebuild vs adaptive on Deep-96 rotation drift."""
...
def _save_results(rows, out_path):
    ...
    fieldnames = [
        "condition", "drift_detected", "ef_search", "epoch_idx", "mean_eh",
        "mean_latency_ms", "mmd_squared", "n_conjugate_edges", "n_index_rebuilds",
        "per_query_recall_std", "queries_per_second", "recall_at_k", "repair_happened",
    ]
...
def run_static(dataset, index_path, config):
    ...
    for epoch_idx, (queries, gt) in enumerate(
        ...
        primary_recall = next(
            r["recall_at_k"] for r in reversed(results) if r["ef_search"] == primary_ef
        )
        print(f"  epoch {epoch_idx:2d}  recall@{primary_ef}={primary_recall:.4f}")

    return results


def run_periodic_rebuild(dataset, index_path, config, condition):
    """Full index rebuild every rebuild_interval epochs; base reloaded each time to avoid double memory."""
    k = config["eval"]["recall_k"]
    ef_values = config["eval"]["ef_search_values"]
    ...
    for epoch_idx, (queries, gt) in enumerate(
        ...
        if epoch_idx > 0 and epoch_idx % rebuild_interval == 0:
            ...
            base_rebuild, _ = load_deep_hdf5(hdf5_path, n_vectors=None)
            index = hnswlib.Index(space="l2", dim=dim)
            index.init_index(
                max_elements=base_rebuild.shape[0],
                ef_construction=idx_cfg["ef_construction"],
                M=idx_cfg["M"],
                random_seed=idx_cfg["seed"],
            )
            index.add_items(base_rebuild, num_threads=-1)
            del base_rebuild
            n_rebuilds += 1
...
def run_all(config_path):
    ...
    config_out = results_dir / "_config.json"
    with open(config_out, "w") as f:
        json.dump(cfg, f, indent=2)
    print(f"Config snapshot saved to {config_out}")
    ...
    for schedule_name, dataset_path in schedules.items():
        ...
        for condition in cfg["conditions"]:
            ...
            if method == "static":
                ...
                rows = run_periodic_rebuild(dataset, index_path, cfg, condition)
