...
#
# Runs standard HNSW search (no_adaptation) over hardness-sorted queries in
# continuous order. The index is loaded once and reused across all ef values
# (safe because no_adaptation never mutates the index).
# Results are saved to out_dir and can be loaded by
# experiment_sift_hardness_poolAndRewire.py so that parameter sweeps over
# adaptation settings never need to re-run this script.
#
# Usage:
#   python experiment_sift_hardness_baseline.py \
...
def save_params(n_index, n_queries_total, dim, bin_edges):
    rows = [
        ...
        {"param": "experiment_type",    "value": "sift_hardness_baseline"},
        {"param": "dim",                "value": dim},
...
def main():
    print(f"SIFT hardness baseline experiment")
    ...
    # Load index once — no_adaptation never mutates it, so all ef values share
    # the same instance instead of reloading per ef (6x fewer index loads).
    print(f"{'='*60}")
    print("strategy: no_adaptation  (single index load, all ef values)")
    print(f"{'='*60}")
