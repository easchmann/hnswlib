# PoolAndRewire adaptation experiment on synthetic data with continuous drift.
#
# Unlike the per-sigma version, a single index (and controller) is reused
# across all shift-sigma levels in order. Queries are fed sequentially:
# warmup at sigma=0, then eval batches for sigma=0,1,2,... on the same
# live index/controller. This mirrors real continuous drift where the query
# distribution shifts gradually over time.
#
# Strategies compared:
...
#
# Usage:
#   python experiment_poolAndRewire_synthetic_continuous.py --mode small
#   python experiment_poolAndRewire_synthetic_continuous.py --mode large --n_init 5000000
...
parser.add_argument("--cluster_spread", type=float, default=1.0)
...
for key in ["n_init", "cluster_std", "cluster_spread", "shift_sigmas", "n_queries", "n_warmup", "ef_sweep",
            "alpha", "max_layer", "max_pool_size", "cooldown", "window_size"]:
    val = getattr(args, key, None)
    if val is not None:
        cfg[key] = val

out_dir = args.out_dir or f"results_poolAndRewire_synthetic_continuous_{args.mode}"
...
def main():
    ...
    print(f"continuous drift mode: index/controller persists across sigma levels\n")
    ...
    for ef in ef_sweep:
        idx = load_fresh(index_path)
        # warmup controller on sigma=0 queries
        warmup_q = query_sets[shift_sigmas[0]][:n_warmup]
        ...
        cumulative_query = 0
        print(f"\n  ef={ef}  (controller initialized on sigma={shift_sigmas[0]} warmup)")
        for sigma in shift_sigmas:
            eval_q  = query_sets[sigma][n_warmup:]
            eval_gt = ground_truths[sigma][n_warmup:]
