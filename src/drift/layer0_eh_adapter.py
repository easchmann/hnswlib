"""Layer-0 edge injection adapter using EH-guided repair with eviction."""
...
class Layer0EHAdapter:
    """EH-guided layer-0 edge injection with farthest-neighbor eviction."""
    ...
    def search_enhanced(self, queries, k, ef_search):
        """Standard HNSW search; layer-0 edges already in graph."""
    ...
    def process_epoch(self, queries, result_ids, result_distances, k):
        """Compute EH, update detector, inject layer-0 edges if drift detected."""
            ...
            if len(repair_nodes) > 0:
                ...
                for src, cand_list in candidates.items():
                    for dst, _ in cand_list:
                        edges_added += self.index.add_layer0_edge_evict(int(src), int(dst))
                self._successful_injections += edges_added

        self.current_epoch += 1
        return {"drift_detected": drift_detected, "edges_added": edges_added}

    def edge_count(self):
        return self._successful_injections
